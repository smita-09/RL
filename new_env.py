...
class JssEnv(gym.Env):
    ...
    def __init__(self, env_config=None):
        ...
        if env_config is None:
            env_config = {'instance_path': str(Path(__file__).parent.absolute()) + '/instances/ta01'}
        ...
        self.dones = None # Newly created attribute
        # initial values for variables used for representation
        self.start_timestamp = datetime.datetime.now().timestamp()
        self.sum_op = 0
        jobs_with_due_dates = list()
        ...
        while line_str:
            ...
            if line_cnt == 1:
                ...
                self.priorities = np.zeros(self.jobs, dtype=np.int)
                # contains all the due dates
                self.due_dates = np.zeros(self.jobs, dtype=np.int)
            else:
                # couple (machine, time)
                assert (len(split_data) - 1) % 2 == 0
                # each jobs must pass a number of operation equal to the number of machines
                assert (len(split_data) - 1) / 2 == self.machines
                i = 0
                ...
                while i < len(split_data)-1:
                    ...
                    self.sum_op += time
                    i += 2
                self.due_dates[job_nb] = split_data[len(split_data)-1] 
                jobs_with_due_dates.append((self.due_dates[job_nb], job_nb))
            line_str = instance_file.readline()
            line_cnt += 1
        instance_file.close()
        print('Due dates are now included', self.due_dates)
        print('This is the list of jobs with their due dates and indices', jobs_with_due_dates)
        jobs_with_due_dates.sort(key=lambda job: job[0])
        for idx, _ in enumerate(jobs_with_due_dates):
            self.priorities[jobs_with_due_dates[idx][1]] = idx
        print('This is now a priority list', self.priorities)
    ...
    def reset(self):
        ...
        self.next_time_step = list() # list containg the assigned jobs
        self.next_jobs = list()
        self.next_priority_list = list()
        ...
        self.dones = np.zeros(self.jobs, dtype=np.int) # newly created to mark whether job is done or not
        self.machine_legal = np.zeros(self.machines, dtype=np.bool)
        print(self.due_dates, self.priorities)
        ...
        if self.nb_machine_legal >= 1:
            for machine in range(self.machines):
                if self.machine_legal[machine]:
                    ...
                    for job in range(self.jobs):
                        if self.needed_machine_jobs[job] == machine and self.legal_actions[job]:
                            if self.todo_time_step_job[job] == (self.machines - 1):
                                ...
                                time_needed_legal = self.instance_matrix[job][current_time_step_non_final][1] # Time taken for non fin op
                    ...
                    if len(non_final_job) > 0: # If there is atleast one job there
                        # This seems bit wrong in a sense that first you are making sure that the length of non_final_job is non zero and then iterating over final_jobs and also in here you are giving priority to final job in this case
    ...
    def _check_no_op(self):
        ...
        self.legal_actions[self.jobs] = False #(Just taking care of the last no op action and making sure that it is false)
            ...
            for job in range(self.jobs):
                if self.legal_actions[job]:
                    ...
                    if end_job < next_time_step:
                        return
                    max_horizon_machine[machine_needed] = min(max_horizon_machine[machine_needed], end_job) #Dont know what is being done here
                    max_horizon = max(max_horizon, max_horizon_machine[machine_needed]) #Dont know why they are being used
            for job in range(self.jobs):
                if not self.legal_actions[job]:
                    if self.time_until_finish_current_op_jobs[job] > 0 and \
                        ...
                        time_step = self.todo_time_step_job[job] + 1 #This is the next time step in the list
                        time_needed = self.current_time_step + self.time_until_finish_current_op_jobs[job] # Kitna time abhi chahiye
                        while time_step < self.machines - 1 and max_horizon > time_needed: # Unless it reaches the last to last operation
                            machine_needed = self.instance_matrix[job][time_step][0] # What is the next machine that will be required
                            if max_horizon_machine[machine_needed] > time_needed and self.machine_legal[machine_needed]:
                                ...
                                if len(machine_next) == self.nb_machine_legal:
                                    self.legal_actions[self.jobs] = True # We are just making sure of the no op job and not anything else,
                                    return
                            time_needed += self.instance_matrix[job][time_step][1]
                            time_step += 1
                    elif not self.action_illegal_no_op[job] and self.todo_time_step_job[job] < self.machines: # This is used for the condition when job is illegal because it is waiting for some machine which is not free 
                        time_step = self.todo_time_step_job[job]
                        machine_needed = self.instance_matrix[job][time_step][0]
    ...
    def step(self, action: int):
        ...
        if action == self.jobs:
            ...
            if(current_time_step_job < self.machines):
                ...
                if to_add_time_step not in self.next_time_step:
                    ...
                    # Now we are bisecting according to their proirotia list
                    # index = bisect.bisect_left(self.next_priority_list, self.priorities[action])
                    # self.next_time_step.insert(index, to_add_time_step)
                    # self.next_jobs.insert(index, action)
                    # self.next_priority_list.insert(index, self.priorities[action])
        ...
        # Iterating over all jobs to give negative rewards to jobs which are overdue and give postive rewards which are finished before their due time
        # Need to give positive rewards but do not know how
            for job in range(self.jobs):
                if self.needed_machine_jobs[job] == -1:
                    print(' Just need to see if this will be called sometimes when the job is actually done')
                    # Added to make sure that negative reward is given when not finished on time
                    if self.dones[job] == 0:
                        if (self.due_dates[job] - self.current_time_step) < 0:
                            print('Is something being done over here', self.current_time_step - self.due_dates[job], self.current_time_step, self.due_dates[job])
                            reward -= (self.current_time_step - self.due_dates[job])*10
                        
                    # Also adding a positive reward to grant positive reward when finished on time  # Not has been working alright cause just need to give postive reward once cause it doenst make sense to just add it over and over again
                        else :
                            reward += (self.due_dates[job] - self.current_time_step)*2
                        self.dones[job] = 1 # Just so that it doesn not calculate the reward over and over again
            scaled_reward = self._reward_scaler(reward)
            return self._get_current_state_representation(), scaled_reward, self._is_done(), {}
                ...
                for job in range(self.jobs):
                    if self.needed_machine_jobs[job] == machine and not self.legal_actions[job] and not \
                        ...
                        if not self.machine_legal[machine]:
                            ...
                            self.nb_machine_legal += 1
        for job in range(self.jobs):
            # Time left to finish the total operation
            time_left = self.jobs_length[job] - self.total_perform_op_time_jobs[job]
            # Expected time to finish the job now
            new_time_to_finish = self.current_time_step + time_left
            # Will it exceed the due date?
            time_due_now = self.due_dates[job] - new_time_to_finish
            print('This is the time left and we are given with new time to finish and then we have time due now', time_left, new_time_to_finish, time_due_now)
            if time_due_now < 0:
                hole_planning += (time_due_now * -1)
                print(hole_planning)
    ...
    def _is_done(self):
        if self.nb_legal_actions == 0:
            ...
            print('Episode is done herewwwwwwww!')
            return True
        return False
